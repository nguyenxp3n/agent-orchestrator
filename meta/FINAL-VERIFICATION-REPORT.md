...
The release is a **Project-Agnostic field manual + operational toolkit + Prompt Engineering System**, not an executable orchestration runtime. Verification focuses on requirement coverage, prompt-contract consistency, model/harness portability, project-neutral hygiene, Markdown link integrity, placeholder hygiene, checksum manifest, and clean re-extraction.

## 2. Source Synthesis

The framework preserves governance from previous runtimes and finalization specs and upgrades the prompt subsystem using the GitHub sources supplied by the user:
...
The principles were synthesized; canonical prompts do not copy upstream prompts verbatim and do not depend on vendor-specific syntax.

## 3. Prompt Engineering Upgrade

### Canonical contract

The simple prompt baseline is retained:

```text
ROLE + CONTEXT + TASK + FORMAT + CONSTRAINTS
```

The agent-grade compiler extends it to:
...
- Project Intake â†’ WP â†’ DAG â†’ Ownership â†’ isolated workspace: preserved.
